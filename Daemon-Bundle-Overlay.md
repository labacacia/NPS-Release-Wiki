...
## docker-compose.yml (at v1.0.0-alpha.15)
...
- CI reads the suite version from the oracle (e.g. `1.0.0-alpha.15`).
...
The current bundle pins the **alpha.15** daemon set, built against `MessagePack 3.1.7`. **alpha.12 was withdrawn** before general use: its NuGet packages shipped the vulnerable `MessagePack 3.0.300` (NU1903). **alpha.13 superseded it**, rebuilding the daemon set against `MessagePack 3.1.7` (carried forward through alpha.14 and alpha.15); operators must skip alpha.12 entirely and pin `1.0.0-alpha.15`.
